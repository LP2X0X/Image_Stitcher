...
MINIMUM_MATCH_POINTS = 4
CONFIDENCE_THRESH = 1 # confidence percentage threshold of match points used for homography computation

def get_matches(img_a_gray, img_b_gray, mask_1, mask_2, img_a, img_b, num_keypoints = 5000, threshold=0.8):
    ...
    '''
    sift = cv2.SIFT_create(nfeatures = num_keypoints, nOctaveLayers = 5, contrastThreshold = 0.0001, edgeThreshold = 35, sigma = 2)
    kp_a, desc_a = sift.detectAndCompute(img_a, mask_1)
    kp_b, desc_b = sift.detectAndCompute(img_b, mask_2)
    # img = cv2.drawKeypoints(img_a, kp_a, img_a, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    # img_ = cv2.drawKeypoints(img_b, kp_b, img_b, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    # cv2.imshow("A", imutils.resize(img_a, height = 1080))
    # cv2.imshow("B", imutils.resize(img_b, height = 1080))
    # cv2.waitKey(0)
...
def calculate_homography(points_img_a, points_img_b):
    ...
    # solution is the last column of v which means the last row of its transpose v_t
...
def compute_homography_ransac(matches_a, matches_b):
    ...
    # RANSAC parameters
    SAMPLE_SIZE = 7 #number of point correspondances for estimation of Homgraphy
    SUCCESS_PROB = 0.99 #required probabilty of finding H with all samples being inliners 
...
def stitch_image_pair(img_a, img_b, stitch_direc):
    ...
    cv2.rectangle(mask_1, (img_a.shape[1] - 300,0), (img_a.shape[1],img_a.shape[0]), (255), thickness = -1)
    cv2.rectangle(mask_2, (0,0), (300,img_b.shape[0]), (255), thickness = -1)
    # cv2.imshow("a", img_1)
    # cv2.imshow("b", img_2)
    # cv2.waitKey(0) 
    matches_a, matches_b = get_matches(img_a_gray, img_b_gray, mask_1, mask_2, img_a, img_b, num_keypoints=10000, threshold=0.4)
